...
app.get('/', (_req, res) => res.status(200).json({ ok: true, service: 'kiau-hoosh' }));
app.get('/health', (_req, res) => res.status(200).json({ ok: true, uptime: process.uptime() }));

// grammY parses and dispatches the Telegram Update to the listeners below.
app.use(webhookPath, webhookCallback(telegramBot, 'express', {
...
async function configureWebhook() {
  const domain = process.env.WEBHOOK_URL || (process.env.RAILWAY_PUBLIC_DOMAIN ? `https://${process.env.RAILWAY_PUBLIC_DOMAIN}` : '');
  if (!domain) {
    console.warn('[telegram] WEBHOOK_URL/RAILWAY_PUBLIC_DOMAIN is not set; webhook was not configured.');
    return;
  }
  ...
  const options = {
    ...
    max_connections: Number(process.env.TELEGRAM_MAX_CONNECTIONS || 40),
  };
  if (webhookSecret) options.secret_token = webhookSecret;
...
const cleanupTimer = setInterval(() => {
  cleanupExpiredSessions().catch((error) => console.error('[db] session cleanup failed:', error));
}, 15 * 60 * 1000);
cleanupTimer.unref();
